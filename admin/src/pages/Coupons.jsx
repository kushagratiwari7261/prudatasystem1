        ...
        <div style={{ display: 'flex' }}>
            ...
            <main style={{ marginLeft: 240, flex: 1, padding: '32px', minHeight: '100vh', background: 'var(--bg-main)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                    <div>
                        <h1 style={{ fontSize: '24px', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '8px' }}>Coupons & Promotions</h1>
                        <p style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>Manage discount codes for customer checkout.</p>
                    </div>
                    <button 
                        onClick={() => setShowAddForm(!showAddForm)}
                        style={{ background: 'var(--text-primary)', color: 'var(--bg-main)', border: 'none', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: 600 }}
                    >
                ...
                {/* Add Coupon Form */}
                {showAddForm && (
                    <div style={{ background: 'var(--bg-card)', padding: '24px', borderRadius: '14px', marginBottom: '24px', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)' }}>
                        <h2 style={{ fontSize: '18px', marginBottom: '20px', color: 'var(--text-primary)' }}>Create New Coupon</h2>
                        <form onSubmit={handleCreateCoupon}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                                <div>
                                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>Coupon Code*</label>
                                    <input 
                                        name="code" value={newCoupon.code} onChange={handleInputChange} 
                                        required style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid var(--border-color)', background: 'var(--input-bg)', color: 'var(--text-primary)' }} 
                                        placeholder="e.g. SUMMER50"
                                    />
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                    <div>
                                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>Type*</label>
                                        <select 
                                            name="discount_type" value={newCoupon.discount_type} onChange={handleInputChange}
                                            style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid var(--border-color)', background: 'var(--input-bg)', color: 'var(--text-primary)' }}
                                        >
                                    ...
                                    <div>
                                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>Discount Value*</label>
                                        <input 
                                ...
                                <div>
                                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>Min Purchase Amt (₹)</label>
                                    <input 
                                ...
                                <div>
                                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>Max Discount Amt (₹) [For %]</label>
                                    <input 
                                ...
                                <div>
                                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>Valid Until Date</label>
                                    <input 
                                ...
                                <div>
                                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>Overall Usage Limit</label>
                                    <input 
                                ...
                                <div>
                                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>Target Product (Optional)</label>
                                    <select 
                            ...
                            <button type="submit" style={{ background: 'var(--accent-blue)', color: 'white', padding: '10px 24px', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>
                                Finalize & Create
                            </button>
                        </form>
                    </div>
                ...
                <div style={{
                    ...
                    {loading ? (
                        ...
                        <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>No coupons found. Create your first promotion above!</div>
                    ) : (
                            ...
                            <tbody>
                                {coupons.map(coupon => (
                                    <tr key={coupon.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                                        <td style={{ padding: '16px', fontSize: '14px', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '1px' }}>
                                            {coupon.code}
                                        </td>
                                        <td style={{ padding: '16px' }}>
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                                {coupon.is_first_purchase_only && <span style={{ background: 'rgba(192, 38, 211, 0.1)', color: '#c026d3', padding: '2px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', width: 'fit-content' }}>FIRST ORDER</span>}
                                                {coupon.product_title && (
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'var(--bg-secondary)', padding: '4px 8px', borderRadius: '4px', width: 'fit-content' }}>
                                                        ...
                                                        <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)' }}>{coupon.product_title}</span>
                                                    </div>
                                                )}
                                                {coupon.min_order_amount > 0 && <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Min: ₹{coupon.min_order_amount}</span>}
                                            </div>
                                        </td>
                                        <td style={{ padding: '16px', fontSize: '14px', color: 'var(--accent-blue)', fontWeight: 'bold' }}>
                                        ...
                                        <td style={{ padding: '16px' }}>
                                            <button 
                                                ...
                                                style={{ padding: '6px 12px', background: 'rgba(239, 68, 68, 0.1)', color: 'var(--accent-red)', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px' }}
                                            >
                                                Delete
                                            </button>
                                        </td>
